...
main();
async function main() {
    const config = (await chrome.storage.sync.get("df-config"));
        ...
        for (const mutation of mutations) {
            if (mutation.type === "childList") {
                for (const node of mutation.addedNodes) {
                    if (node.textContent?.includes(config.facebook?.suggestionCaptureText || "Suggested for you") &&
                        ...
                        const nodeObserver = new MutationObserver((nodeMutations) => {
                            for (const nodeMutation of nodeMutations) {
                                for (const maybeVideoContainer of nodeMutation.addedNodes) {
                                    ...
                                    const videoContainer = videoElement.closest(config.facebook?.videoContainerSelector ||
                                        "div[data-visualcompletion]")?.parentElement;
                        ...
                        for (const img of allImages) {
                            if (img.width > 300) {
                                addBlur(img, config);
                                const imgContainer = img.closest("a")?.parentElement?.parentElement;
                                if (!imgContainer) {
                                    continue;
                                }
...
function observeForVideoThumbnail(videoContainer, config) {
